 ...
 * 优点：
 * <ul>
 *   <li>便于精细控制。你可以为某一种具体情况指定相应的规则，而几乎不会影响运行时的性能</li>
 * </ul>
 */
public class DecisionMap<T> {
	...
	public DecisionMap (List<DecisionFactor> factorList, Map<Integer, Supplier<T>> strategyMap, Supplier<T> defaultOperation) {
		if (factorList.size() > MAX_FACTOR_COUNT) {
			throw new IllegalArgumentException("Too many factors. Max is " + MAX_FACTOR_COUNT + ", got " + factorList.size());
		}
